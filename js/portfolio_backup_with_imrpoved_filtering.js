...
class ArtworkCategorizer {
    ...
    // NEW: Multi-select filter system
        ...
        return artworks.filter(artwork => {
            ...
            for (const [filterType, filterValues] of Object.entries(activeFilters)) {
                ...
                switch(filterType) {
                    ...
                    case 'category':
                        matches = filterValues.some(value => {
                            if (value === 'recent') return this.checkYearFilter(artwork.year, 'recent');
                            if (value === 'featured') return artwork.featured === true || artwork.featured === 'true';
                            return artwork.category === value;
                        });
                        break;
                    default:
                        matches = true;
                ...
                // If any filter type doesn't match, exclude this artwork
                if (!matches) {
                    return false;
                }
            }
